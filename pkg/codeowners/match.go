...
func (pattern Pattern) Matches(path string) (bool, error) {
	...
	// effect, no matter where they are defined. Put a backslash ("\") in front of the
	...
	// directory; however frotz/ matches frotz and a/frotz that is a directory (all paths
	// are relative from the .gitignore file).

	// An asterisk "*" matches anything except a slash. The character "?" matches any
	// one character except "/". The range notation, e.g. [a-zA-Z], can be used to match
	// one of the characters in a range. See fnmatch(3) and the FNM_PATHNAME flag for a
	// more detailed description.

	return false, nil
}

func matchPatternWithConsecutiveAsterisks(pattern, path string) (bool, error) {
